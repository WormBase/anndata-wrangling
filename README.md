...
| `adata.obs.cell_subtype` | The cell subtype annotation if provided by the authors. If the annotation contains individual neuron types, these should be cell subtypes. If only a cell type label was provided, it can be repeated in both `cell_type` and `cell_subtype`| categorical | `ASJ`|Required|
